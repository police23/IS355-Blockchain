    ...
    const purchaseVoucher = async (userID, voucherID) => {
        try {
            ...
            if(res.success) {
                ...
                const newMyVouchersData = data.userVouchers.map(uv => ({
                    id: uv.id,
                    code: uv.code,
                    discount: uv.VoucherType?.discount_percent || 0,
                    typeName: uv.VoucherType?.name || 'Unknown',
                            ...
                            <div className="vouchers-grid">
                                {myVouchers.map((voucher) => (
                                    <div key={voucher.id} className="voucher-card my-voucher">
                                        <div className={`voucher-badge ${getStatusBadgeClass(voucher.status)}`}>
                                            {getStatusLabel(voucher.status)}
                                        </div>
                                        <div className="voucher-content">
                                            <h3>{voucher.code}</h3>
                                            <p className="voucher-discount">{voucher.discount}% OFF</p>
                                            <p className="voucher-type">{voucher.typeName}</p>
                                            {voucher.expiresAt && (
                                                <p className="voucher-expires">Hết hạn: {new Date(voucher.expiresAt).toLocaleDateString('vi-VN')}</p>
                                            )}
                                        </div>
                                    </div>
                                ))}
                        ...
                        {availableVouchers.length === 0 ? (
                            ...
                            <div className="vouchers-grid">
                                {availableVouchers.map((voucher) => (
                                    <div key={voucher.id} className="voucher-card available-voucher">
                                        <div className="voucher-header-section">
                                            <h3>{voucher.name}</h3>
                                            <span className="token-cost">{voucher.tokenCost} tokens</span>
                                        </div>
                                        <div className="voucher-content">
                                            <p className="voucher-discount">{voucher.discountPercent}% OFF</p>
                                            <p className="voucher-description">{voucher.description}</p>
                                            <button className="purchase-btn">Mua ngay</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
